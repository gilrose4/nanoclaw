...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    ...
    // Check for recently written log file (written by nanoclaw on startup)
    const logFile = path.join(projectRoot, 'logs', 'nanoclaw.log');
    if (fs.existsSync(logFile)) {
      const stat = fs.statSync(logFile);
      const ageMs = Date.now() - stat.mtimeMs;
      // Running if log was written in the last 5 minutes
      service = ageMs < 5 * 60 * 1000 ? 'running' : 'stopped';
    }
  } else {
